...
<rules>
1. СТИЛЬ: Живой, динамичный, с легким криминальным оттенком (как Crime News). Используй короткие рубленые фразы.
2. ЭМОДЗИ: В начале поставь ОДИН тематический эмодзи (⚖️ для судов, 🚔 для криминала, 💀 для ЧП, 🌍 для политики).
...
6. ГОРОД: Определи, о каком городе/регионе идет речь. Если новость федеральная (без привязки к месту) — пиши "Федеральные".
7. ХЭШТЕГ: В самом конце текста добавь хэштег города в формате: #Москва, #СПб, #Казань, #Федеральные и т.д.
</rules>
...
Формат ответа: [эмодзи] [текст новости] [хэштег]
"""
    
    # ... остальной код функции не меняй, только промпт
